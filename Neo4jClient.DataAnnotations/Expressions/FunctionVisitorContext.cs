...
namespace Neo4jClient.DataAnnotations.Expressions
{
    public class FunctionVisitorContext : IHaveAnnotationsContext
    {
        ...
        public Dictionary<Type, List<Func<FunctionHandlerContext, Func<Expression>>>> TypeHandlers { get; set; } =
            FunctionExpressionVisitor.DefaultTypeHandlers;

        public Dictionary<ExpressionType, List<Func<FunctionHandlerContext, Func<Expression>>>> NodeTypeHandlers
        {
            get;
            set;
        } = FunctionExpressionVisitor.DefaultNodeTypeHandlers;

        public Dictionary<MemberInfo, Func<FunctionHandlerContext, Func<Expression>>> MemberInfoHandlers { get; set; } =
            FunctionExpressionVisitor.DefaultMemberInfoHandlers;

        public Dictionary<ExpressionType, string> BinaryOperators { get; set; } =
            FunctionExpressionVisitor.DefaultBinaryOperators;

        public Dictionary<ExpressionType, string> LeftAlignedUnaryOperators { get; set; } =
            FunctionExpressionVisitor.DefaultLeftAlignedUnaryOperators;

        public Dictionary<ExpressionType, string> RightAlignedUnaryOperators { get; set; } =
            FunctionExpressionVisitor.DefaultRightAlignedUnaryOperators;
        ...
        public Func<object, string> GetSerializer()
        {
            return Visitor?.Serializer;
        }
    }
}