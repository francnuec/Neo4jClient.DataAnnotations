...
namespace Neo4jClient.DataAnnotations.Tests
{
    public class EntityExpressionVisitorTests
    {
        ...
        public void AnonymousType(string testContextName, TestContext testContext)
        {
            ...
            Assert.Single(entityVisitor.PendingAssignments); //because we are accounting for an implicit convert to object type.
        ...
        public void ComplexAnonymousType(string testContextName, TestContext testContext)
        {
            ...
            Assert.Contains("Get(\"A\").get_Item(\"Location_Latitude\")", entityVisitor.PendingAssignments.First().Value.ToString());

            Assert.Equal("Location_Longitude", entityVisitor.PendingAssignments.Last().Key.ComplexJsonName);
            Assert.Contains("Get(\"A\").get_Item(\"AddressLine\")", entityVisitor.PendingAssignments.Last().Value.ToString());
        ...
        public void EscapedComplexAnonymousType(string testContextName, TestContext testContext)
        {
            ...
            Assert.Contains("Get(\"shondaRhimes\").get_Item(\"NewAddressName_Location_Longitude\")", entityVisitor.PendingAssignments.First().Value.ToString());
        ...
        public void Predicate(string testContextName, TestContext testContext)
        {
            ...
            Assert.Equal("Born", entityVisitor.PendingAssignments.First().Key.ComplexJsonName);
            Assert.Equal("Get(\"ellenPompeo\").Born._()", entityVisitor.PendingAssignments.First().Value.ToString()); //the npf escape will be added for concrete classes
        ...
        {
            Expression<Func<ActorNode, bool>> expression = a => (a.Address as AddressWithComplexType).Location.Longitude == new AddressWithComplexType()
            ...
            Assert.Equal("NewAddressName_Location_Longitude", entityVisitor.PendingAssignments.First().Key.ComplexJsonName);
        ...
        public void PredicateDictionarySet(string testContextName, TestContext testContext)
        {
            ...
            Assert.Equal(3, entityVisitor.PredicateAssignments.Count);
            Assert.Equal(4, entityVisitor.PendingAssignments.Count); //the address complex type should have exploded to make 4

            Assert.Equal("NewAddressName_AddressLine", entityVisitor.PendingAssignments.First().Key.ComplexJsonName);
            Assert.Contains("Get(\"ellenPompeo\").Address.AddressLine", entityVisitor.PendingAssignments.First().Value.ToString());
