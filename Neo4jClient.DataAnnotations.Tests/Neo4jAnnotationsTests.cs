...
namespace Neo4jClient.DataAnnotations.Tests
{
    public class Neo4jAnnotationsTests
    {
        ...
        {
            Assert.All(TestUtilities.EntityTypes,
                type => Assert.True(testContext.AnnotationsContext.EntityService.EntityTypes.Contains(type)));

            Assert.Equal(typeof(EntityResolver), testContext.QueryContext.Client.JsonContractResolver.GetType());

            Assert.Contains(testContext.QueryContext.Client.JsonConverters,
                c => typeof(EntityResolverConverter) == c.GetType());
        }
        ...
        public void RegisterWithConverterImplicit(string testContextName, TestContext testContext)
        {
            ...
            Assert.Contains(testContext.QueryContext.Client.JsonConverters,
                c => typeof(EntityConverter) == c.GetType());
        ...
        public void RegisterWithResolver(string testContextName, TestContext testContext)
        {
            ...
            Assert.True(testContext.QueryContext.Client.JsonContractResolver ==
                        testContext.AnnotationsContext.EntityResolver);
        ...
        public void RegisterWithConverter(string testContextName, TestContext testContext)
        {
            ...
            Assert.True(testContext.QueryContext.Client.JsonConverters.Last() ==
                        testContext.AnnotationsContext.EntityConverter);
        }
    }
}